...
Copyright     : Copyright (c) 2007-2008 by FHNW. All rights reserved.
*******************************************************************************/
...
public:
   ...
   inline bool IsBetweenPoints(const vec3<T>& begin, const vec3<T>& end) const
   {
   ...
   inline vec3<T> GetInterpolated(const vec3<T>& other, T d) const
   {
      T inv = (T)1.0 - d;
      return vec3<T>(other.x*inv + x*d, other.y*inv + y*d, other.z*inv + z*d);
   }

   //! \brief Rotate vector around axis. Angle is given in degree.
   inline void Rotate(T angle, const vec3<T> u)
   {
      T rx,ry,rz;
      T cost = cos((T)deg2rad(angle)); T sint = sin((T)deg2rad(angle));
      T ux2 = u.x*u.x; T uy2 = u.y*u.y; T uz2=u.z*u.z;
      rx = x*(ux2+cost*(1-ux2))  + y*(u.x*u.y*(1-cost)+u.z*sint) + z*(u.z*u.x*(1-cost)-u.y*sint);
      ry = x*(u.x*u.y*(1-cost)-u.z*sint) + y*(uy2+ cost*(1-uy2))+ z*( u.y*u.z*(1-cost)+u.x*sint);
      rz = x*(u.z*u.x*(1-cost)+u.y*sint) + y*(u.y*u.z*(1-cost)-u.x*sint) + z*(uz2+cost*(1-uz2));
      x = rx; y=ry; z=rz;
   }

   //! Test if point is inside box
   //! \deprecated Will be removed. Use math::pointboxtest instead.
   inline bool IsInBox(const vec3<T>& b1, const vec3<T>& b2) const
   {
      if ((x>=b1.x) && (y>=b1.y) && (z>=b1.z) && (x<=b2.x) && (y<=b2.y) && (z<=b2.z)) return true;
      return false;
   }

   //! \brief Return axis with max dimension
   ...
   inline void setMin(vec3<T> other)                                        
   {                                                                      
      x = MIN(x,other.x);                                            
      y = MIN(y,other.y);                                            
      z = MIN(z,other.z);                                            
   ...
   inline void setMax(vec3<T> other)                                        
   {                                                                      
      x = MAX(x,other.x);                                            
      y = MAX(y,other.y);                                            
      z = MAX(z,other.z);                                            
