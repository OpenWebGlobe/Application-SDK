...
double MercatorQuadtree::GroundResolution(double latitude, int levelofdetail)
{
   ...
   return cos(latitude * AGEPI / 180.0) * 2 * AGEPI * _EarthRadius / MapSize(levelofdetail);
}
...
void MercatorQuadtree::WGS84ToPixel(double lng, double lat, int levelofdetail, int64& outPixelX, int64& outPixelY)
{
   ...
   double out_x, out_y;
   GeoRef::Mercator::ForwardCustom(lng, lat, out_x, out_y, _ellipsoid_e);
...
{
   double outx, outy;
   GeoRef::Mercator::ForwardCustom(x,y, outx, outy, _ellipsoid_e);
   x = outx;
   y = outy;
}
...
void MercatorQuadtree::MercatorToPixel(double x, double y, int levelofdetail, int64& outPixelX, int64& outPixelY)
{
   ...
   outPixelX = (int64) math::Clip<int64>( normX * (mapsize-1) + 0.5, 0, mapsize-1);
   outPixelY = (int64) math::Clip<int64>( normY * (mapsize-1) + 0.5, 0, mapsize-1);
